...
const onChange: TableProps<Blog>["onChange"] = (
  pagination,
  filters,
  sorter,
  extra
...
const BlogManage: React.FC = () => {
  ...
  const [productList, setProductList] = useState<Blog[]>([]);
  ...
  const onChange: TableProps<Blog>["onChange"] = (pagination) => {
    setFilters((prev) => ({
      ...prev,
  ...
  const showModalEdit = (isOpen: boolean, data: Blog) => {
    setModalEdit({
      isOpen,
      data,
    });
  ...
  return (
    <div>
      <div className="flex items-center justify-end my-4 space-x-2">
        ...
        >
          <BlogForm initForm={modalEdit.data} />
        </Modal>
      </div>
      <Table
